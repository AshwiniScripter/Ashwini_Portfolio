...
const Activities = () => {
  const education = [
    {
      title: "High School",
      institution: "AshokDada Sable High School Mangoan, Raigad",
      year: "2021",
      icon: (
        <svg
    ...
    
    {
      ...
      institution: "Vishwakarma Institue of Information Technology",
      year: "2024 - Ongoing",
      icon: (
  ...
  const renderCard = (data) => (
    <div
      className="flex flex-col items-center bg-gray-900 rounded-2xl p-6 shadow-lg transition-transform transform hover:scale-105 hover:shadow-2xl hover:bg-gray-800 border border-gray-100"
    >
      <div className="mb-4">{data.icon}</div>
      <h3 className="text-xl font-bold text-white">{data.title}</h3>
      <p className="text-sm text-gray-400">{data.institution || data.company}</p>
      <span className="text-sm text-yellow-400">{data.year || data.duration}</span>
    </div>
  );

  return (
    <section id="education" className="bg-gray-800 py-10 px-5">
      {/* Education Section */}
      <div className="w-full mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {education.map((edu, index) => (
            <React.Fragment key={index}>{renderCard(edu)}</React.Fragment>
          ))}
        </div>
      </div>
      ...
      <div className="max-w-6xl mx-auto mt-12">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {experience.map((exp, index) => (
            <React.Fragment key={index}>{renderCard(exp)}</React.Fragment>
          ))}
        </div>
      </div>
    </section>
